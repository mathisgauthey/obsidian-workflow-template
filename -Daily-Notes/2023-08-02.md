...
> group by function (!task.due.moment) ? '%%4%% ==Undated==' : result = task.due.moment.isBefore('2023-08-02', 'day') ? '%%1%% ==Overdue==' : result = task.due.moment.isSame('2023-08-02', 'day') ? '%%2%% ==Today==' : '%%3%% ==Future=='
> group by happens
> ```
...
> is not recurring
> path does not include 2-Areas
> happens in 2023-08-03 2023-09-01
> group by happens
> ```
...
> not done
> is recurring
> filter by function task.due.moment?.isSameOrBefore('2023-08-02', 'day') || false
> group by filename 
> ```
